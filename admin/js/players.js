...
$('#player_edit_modal').on('show.bs.modal', function (event) {
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            modal.find('.modal-body #focus-x').val(obj.player_focus_x__blazeweb);
            modal.find('.modal-body #focus-y').val(obj.player_focus_y__blazeweb);
            modal.find('.modal-body #player-id').val(obj.player_id__blazeweb);
            modal.find('.modal-body #player-background').val(obj.player_background_position__blazeweb);
            modal.find('.modal-body #player-image').attr("src","../img/players/"+obj.player_image__blazeweb);
            modal.find();
        },
        error: function(err){
            console.log(error);
